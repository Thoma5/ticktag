...
class TicketUserRelationApiTest : ApiBaseTest() {
    ...
    fun `create and get ticketAssignment with User as ProjectUser should succeed`() {
        withUser(USER_USER_ID) { principal ->
            ticketUserRelationController.createTicketAssignment(TICKET_PROJECT_AOU_OAU_0_ID, ASSTAG_PROJECT_AOU_OAU_IDS.first(), USER_USER_ID)
            ticketUserRelationController.getTicketAssignment(TICKET_PROJECT_AOU_OAU_0_ID, ASSTAG_PROJECT_AOU_OAU_IDS.first(), USER_USER_ID)
        }
    }
    ...
    fun `create and get ticketAssignment with Observer as ProjectObserver should fail`() {
        withUser(OBSERVER_USER_ID) { principal ->
            ticketUserRelationController.createTicketAssignment(TICKET_PROJECT_AOU_UOA_2_ID, ASSTAG_PROJECT_AOU_UOA_IDS.first(), USER_USER_ID)
            ticketUserRelationController.getTicketAssignment(TICKET_PROJECT_AOU_UOA_2_ID, ASSTAG_PROJECT_AOU_UOA_IDS.first(), USER_USER_ID)
        }
    }
    ...
    fun `create and get ticketAssignment with User as ProjectObserver should fail`() {
        withUser(USER_ID) { principal ->
            ticketUserRelationController.createTicketAssignment(TICKET_PROJECT_AOU_AUO_2_ID, ASSTAG_PROJECT_AOU_AUO_IDS.first(), USER_USER_ID)
            ticketUserRelationController.getTicketAssignment(TICKET_PROJECT_AOU_AUO_2_ID, ASSTAG_PROJECT_AOU_AUO_IDS.first(), USER_USER_ID)
        }
    }
    ...
    fun `create ticketAssignment with others projects assignmentTag should fail`() {
        withUser(ADMIN_USER_ID) { principal ->
            ticketUserRelationController.createTicketAssignment(TICKET_PROJECT_AOU_UOA_2_ID, ASSTAG_PROJECT_AOU_OAU_IDS.first(), USER_USER_ID)
        }
    }
    ...
    fun `create ticketAssignment with user who isnt in project should fail`() {
        withUser(ADMIN_USER_ID) { principal ->
            ticketUserRelationController.createTicketAssignment(TICKET_NO_MEMBERS_ID, ASSTAG_PROJECT_NO_MEMBERS_IDS.first(), USER_USER_ID)
        }
    }
    ...
    fun `assign ticket to Admin as ProjectObserver should fail`() {
        withUser(ADMIN_USER_ID) { principal ->
            ticketUserRelationController.createTicketAssignment(TICKET_PROJECT_AOU_OAU_0_ID, ASSTAG_PROJECT_AOU_OAU_IDS.first(), ADMIN_USER_ID)
        }
    }
    ...
    fun `get ticketAssignment with Observer as ProjectObserver should succeed`() {
        withUser(OBSERVER_USER_ID) { principal ->
            ticketUserRelationController.getTicketAssignment(TICKET_PROJECT_AOU_UOA_2_ID, ASSTAG_PROJECT_AOU_UOA_IDS.first(), ADMIN_USER_ID)
        }
    }
    ...
    fun `get ticketAssignment with Admin as ProjectAdmin should succeed`() {
        withUser(ADMIN_USER_ID) { principal ->
            ticketUserRelationController.getTicketAssignment(TICKET_PROJECT_AOU_AUO_2_ID, ASSTAG_PROJECT_AOU_AUO_IDS[1], USER_USER_ID)
        }
    }
    ...
    fun `get ticketAssignment with User as ProjectObserver should succeed`() {
        withUser(USER_USER_ID) { principal ->
            ticketUserRelationController.getTicketAssignment(TICKET_PROJECT_AOU_AUO_2_ID, ASSTAG_PROJECT_AOU_AUO_IDS.first(), ADMIN_USER_ID)
        }
    }
    ...
    fun `delete ticketAssignment with User as ProjectObserver should fail`() {
        withUser(USER_USER_ID) { principal ->
            ticketUserRelationController.deleteTicketAssignment(TICKET_PROJECT_AOU_AUO_2_ID, ASSTAG_PROJECT_AOU_AUO_IDS[1], USER_USER_ID)
    ...
    fun `create get delete and get ticketAssignment with User as ProjectUser should fail`() {
        withUser(USER_USER_ID) { principal ->
            try {
                ticketUserRelationController.createTicketAssignment(TICKET_PROJECT_AOU_OAU_0_ID, ASSTAG_PROJECT_AOU_OAU_IDS.first(), USER_USER_ID)
                ticketUserRelationController.getTicketAssignment(TICKET_PROJECT_AOU_OAU_0_ID, ASSTAG_PROJECT_AOU_OAU_IDS.first(), USER_USER_ID)
                ticketUserRelationController.deleteTicketAssignment(TICKET_PROJECT_AOU_OAU_0_ID, ASSTAG_PROJECT_AOU_OAU_IDS.first(), USER_USER_ID)
            } catch (e: Exception) {
                fail()
            }
            ticketUserRelationController.getTicketAssignment(TICKET_PROJECT_AOU_OAU_0_ID, ASSTAG_PROJECT_AOU_OAU_IDS.first(), USER_USER_ID)
